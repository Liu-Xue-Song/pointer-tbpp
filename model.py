...
class Encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        output = self.conv1(torch.relu(output))
        output = self.conv2(torch.relu(output))
        output = self.conv3(torch.relu(output))
...
class DRL4TSP(nn.Module):
    ...
    def forward(self, static, dynamic, decoder_input=None, last_hh=None, task='tbpp_item'):
        """
        Parameters
        ----------
        ...
        static_origin = static.clone()
        static = static.float()
        for i in range(static.size()[0]):
            max_end = static[i, 1, :].max()
            static[i, 0, :] = static[i, 0, :] / max_end
            static[i, 1, :] = static[i, 1, :] / max_end
            if input_size>2:
                static[i, 2, :] = static[i, 2, :] / 100
        if decoder_input is None:
            decoder_input = self.x0.expand(batch_size, -1, -1).to(device)

        # Always use a mask - if no function is provided, we don't update it
        mask = torch.zeros(batch_size, sequence_size, device=device)
        mask[:,1] = 1
        tracker = np.zeros((batch_size, sequence_size, sequence_size))
        tracker = np.zeros((batch_size, sequence_size))
        # tracker = [[[[0] for i in range(sequence_size)] for j in range(sequence_size)] for k in range(batch_size)]
        # Structures for holding the output sequences
        tour_idx, tour_logp = [], []
        max_steps = sequence_size if input_size==3 else 1000
        if task=='tbpp_item':
            tracker = np.zeros((batch_size, sequence_size))
            max_steps = 1000
        ...
        for _ in range(max_steps):
            if self.update_fn is not None:
                dynamic, mask = self.update_fn(tour_idx, static_origin, _, mask, dynamic, tracker)
                mask.detach_()
                dynamic_hidden = self.dynamic_encoder(dynamic)
            elif self.mask_fn is not None:
                mask = self.mask_fn(tour_idx, static_origin, _).detach()
            if mask.eq(0).all():
                break
            # print(mask)
